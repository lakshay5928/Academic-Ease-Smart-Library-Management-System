...
def filter_courses_by_department(courses, department=None):
    if not department:
        return courses
    return [course for course in courses if course['code'].startswith(department.upper())]

def schedule_courses(selected_courses):
    sorted_courses = sorted(
        selected_courses,
        key=lambda x: (-int(x['credits']), len(x.get('prerequisites', [])))
    )
    scheduled = []
    time_slots = set()
...
def generate_weekly_timetable(scheduled_courses):
    ...
    subject_counts = {course['code']: 0 for course in scheduled_courses}
    max_lectures = 4
    all_courses = scheduled_courses[:]

    for day in timetable:
        day_schedule = []
        used_courses_today = set()
        ...
        for time_slot, slot_type in time_slots:
            if 'Break' in slot_type or 'Lunch' in slot_type:
                ...
                for course in all_courses:
                    code = course['code']
                    if subject_counts[code] < max_lectures and code not in used_courses_today:
                        day_schedule.append({
                            'time': time_slot,
                            'code': code,
                            'name': course['name'],
